...
import { exchangeGoogleCode, googleReturnTo } from "@/lib/auth/google";
...
export async function GET(request: Request) {
  ...
  if (!state || !cookie || state !== cookie) {
    return fail("구글 로그인 요청이 만료되었어요. 다시 시도해 주세요.");
  }
  ...
  try {
    ...
    return fail("구글 로그인 요청을 확인할 수 없습니다.");
  }

  if (intent.termsAccepted !== true || intent.privacyAccepted !== true) {
    return fail("이용약관과 개인정보 처리방침에 동의해 주세요.");
  }

  if (url.searchParams.has("error")) {
    return fail("구글 로그인이 취소되었어요.");
  }

  const code = url.searchParams.get("code");
  if (!code) {
    return fail(
      url.searchParams.get("error_description") ?? "구글 인증이 취소되었어요.",
    );
  }
  ...
  try {
    ...
    const response = NextResponse.redirect(
      new URL(
        postLoginRoute(
          googleReturnTo(intent.returnTo),
    ...
    return fail("구글 로그인에 실패했어요. 잠시 후 다시 시도해 주세요.");
  }
}
